...
async function fetchVideos() {
    ...
    try {
        const response = await fetch(`https://downloader-services.onrender.com/youtube/search?query=${encodeURIComponent(query)}`);
        if (!response.ok) {
            throw new Error("सर्वर से गलत प्रतिक्रिया मिली");
        }
        const data = await response.json();
...
function startDownload(format) {
    window.location.href = `https://downloader-services.onrender.com/download/url?url=${encodeURIComponent(selectedVideoUrl)}&format=${format}`;
    closeModal();
}
